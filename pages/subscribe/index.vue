<template>
  <v-container class="text-center">
    <div v-motion :initial="{ opacity: 0, y: 30 }" :enter="{ opacity: 1, y: 0, transition: { duration: 600 } }">
      <h2 class="text-h5 font-weight-bold gradient-text-alt outfit outfit-bold">Subscribe to Generate Unlimited Quizzes</h2>
    ...
    <p class="mt-6 text-caption outfit outfit-regular" v-motion :initial="{ opacity: 0 }" :enter="{ opacity: 1, transition: { delay: 800, duration: 500 } }">
      Visit our <a href="#" class="text-decoration-none link-hover" style="color: blue;">plans page</a> to compare all features.
    </p>
  </v-container>
</template>
...
// Add animation trigger on tab change
watch(selectedTab, () => {
  // Animation will be handled by Vue's transition system
});
</script>
