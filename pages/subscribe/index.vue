<template>
  <v-container class="text-center">
    ...
    <!-- Toggle Buttons -->
    <v-btn-toggle v-model="selectedTab" class="mt-3 bg-grey-lighten-3 rounded-pill pa-1" mandatory>
      <v-btn
        value="school"
        class="rounded-pill"
        :class="selectedTab === 'school' ? 'bg-deep-purple-lighten-4 text-black' : 'bg-transparent text-black'"
      >
        School
      </v-btn>
      <v-btn
        value="work"
        class="rounded-pill"
        :class="selectedTab === 'work' ? 'bg-deep-purple-lighten-4 text-black' : 'bg-transparent text-black'"
      >
        Work
      </v-btn>
    </v-btn-toggle>
      ...
      <v-col cols="12" md="5" v-for="(plan, index) in activePlans" :key="index">
        <v-card class="pa-6" elevation="2" style="border-radius: 20px">
          ...
          <v-list density="compact">
            <v-list-item v-for="(feature, i) in plan.features" :key="i" class="d-flex align-center">
              <template #prepend>
                <img src="/icons/icons8-checkmark.svg" alt="Checkmark" class="icon-size" />
              </template>
              <v-list-item-content>
          ...
          <!-- Subscribe Button -->
          <v-btn block :color="plan.buttonColor" class="mt-4" round :variant="getButtonVariant(plan.name)">
            Subscribe
          </v-btn>
        </v-card>
      </v-col>
...
const getCheckmarkIcon = (colorClass) => {
  return colorClass === "text-primary"
    ? "/icons/checkmark-blue.svg"
    : "/icons/checkmark-red.svg";
};
