<!-- <template>
  ...
  window.onTelegramAuth = (user) => {
   ...
   <template>
    <div>
      <!-- Hidden Telegram Login Widget -->
      <div id="telegram-login-container" class="hidden-telegram-button"></div>
  ...
  const botUsername = "owlmingo_bot"; // Your bot username
  ...
  // Function to trigger Telegram login when clicking the custom button
  const triggerTelegramLogin = () => {
    const iframe = document.querySelector("#telegram-login-container iframe");
    if (iframe) {
      iframe.style.display = "block"; // Show hidden iframe
      iframe.contentWindow.postMessage("login", "*"); // Try to trigger login
    } else {
      console.error("Telegram login iframe not found!");
    }
  };
  ...
  <style scoped>
  /* Hide the default Telegram button */
