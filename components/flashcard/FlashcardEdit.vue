<template>
  <div>
    ...
    <v-container class="text-center py-6 d-flex align-center justify-space-between">
      ...
      <v-btn color="info" variant="outlined" rounded="xl" @click="createDeck">
        Create
      </v-btn>
    </v-container>
    ...
    <v-container>
      <v-card class="pa-4">
        ...
        <v-card-text>
          <v-textarea label="Flashcard Deck Name" placeholder="Enter the name of the flashcard deck" variant="outlined"
            v-model="deckName" auto-grow rows="2" hide-details clearable></v-textarea>
          <p class="text-grey-darken-1 text-body-2">All changes are saved automatically. Click and drag to rearrange
            flashcards.</p>
        </v-card-text>
      </v-card>
    </v-container>
          ...
          <v-row class="align-center mt-3">
            ...
            <v-col cols="12" sm="6" class="d-flex align-center">
              <v-card class="image-placeholder mr-2" @click="selectImage(index, 'front')">
                <input type="file" :ref="(el) => setFileInputRef(index, 'front', el)"
                  @change="onImageChange($event, index, 'front')" accept="image/*" hidden />
            ...
            <v-col cols="12" sm="6" class="d-flex align-center">
              ...
              <div contenteditable="true" class="editable-textarea" :innerHTML="flashcard.backText"
                @input="flashcard.backText = $event.target.innerHTML" placeholder="Enter back text"></div>
              <v-card class="image-placeholder ml-2" @click="selectImage(index, 'back')">
                <input type="file" :ref="(el) => setFileInputRef(index, 'back', el)"
                  @change="onImageChange($event, index, 'back')" accept="image/*" hidden />
...
const applyFormat = (format) => {
  ...
  if (format === "unordered-list") span.style.listStyleType = "disc";
  if (format === "ordered-list") span.style.listStyleType = "decimal";
