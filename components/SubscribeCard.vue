<template> 
    <v-container class="text-center">
      ...
      <p class="text-body-2 text-grey-darken-1">
        ...
        From training materials to academic notes, our AI intelligently creates quizzes to 
        reinforce learning or assess knowledge for students and professionals alike.
      </p>
  
      <!-- School & Work Toggle -->
      <v-btn-toggle v-model="selectedTab" 
        class="mt-3 bg-grey-lighten-3 rounded-pill pa-1" 
        density="compact" 
        mandatory
      >
        <v-btn value="school" 
          class="rounded-pill" :class="{'bg-blue-lighten-4 text-black': selectedTab === 'school', 
          'bg-transparent text-black': selectedTab !== 'school'}" size="small"
        >
          School
        </v-btn>
        <v-btn value="work" 
          class="rounded-pill" :class="{'bg-blue-lighten-4 text-black': selectedTab === 'work', 
          'bg-transparent text-black': selectedTab !== 'work'}" size="small"
        >
          Work
        </v-btn>
      </v-btn-toggle>
      ...
      <v-row class="mt-6 justify-center">
        <v-col cols="12" md="5" v-for="(plan, index) in activePlans" :key="index">
          <v-card class="pa-6 text-left rounded-lg" elevation="2">
            <p class="text-h4 font-weight-bold mb-1" :class="plan.colorClass">{{ plan.name }}</p>
            ...
            <p class="text-h4 font-weight-bold mb-1">
              ...
              <template v-else>
                {{ selectedTypes[plan.name] === 'month' ? plan.monthlyPrice : plan.annualPrice }} <span class="text-h6">/ month</span>
              </template>
            </p>
            ...
            <!-- Work Plan Toggle (Only for Work Plans) -->
            <transition name="fade">
              <v-btn-toggle v-if="selectedTab === 'work'" v-model="selectedTypes[plan.name]" 
                class="mt-2 bg-grey-lighten-3 rounded-pill pa-1" density="compact" mandatory>
                <v-btn class="rounded-pill" size="small" value="annual" 
                  :class="{'bg-grey-lighten-2 text-black': selectedTypes[plan.name] === 'annual', 
                  'bg-transparent text-black': selectedTypes[plan.name] !== 'annual'}"
                >
                  Annual
                </v-btn>
                <v-btn class="rounded-pill" size="small" value="month" 
                  :class="{'bg-grey-lighten-2 text-black': selectedTypes[plan.name] === 'month', 
            ...
            <v-list class="text-left">
              <v-list-item v-for="(feature, i) in plan.features" :key="i" class="align-start">
                <template #prepend>
                  <img src="/public/icons/icons8-checkmark.svg" alt="Checkmark" 
                    style="height: 30px; width: 30px; margin-right: 5px;" />
                </template>
                <v-list-item-content>
                  <div style="display: flex; align-items: center; gap: 8px;">
                    <v-list-item-title 
                      :style="{ fontWeight: selectedTab === 'work' && (i === plan.features.length - 1 || i === plan.features.length - 2) ? 'bold' : 'normal' }"
                    >{{ feature }}
                    </v-list-item-title>
  
                    <!-- Tooltip for features -->
                    <v-tooltip location="top" v-if="i === 1">
                      <template v-slot:activator="{ props }">
                        <v-icon small v-bind="props" style="cursor: pointer; display: flex; align-items: center;">
                          mdi-information-outline
                        </v-icon>
                      </template>
                      <div style="text-align: center; max-width: 310px;">
                        For questions that require an open-ended answer, artificial intelligence will determine 
                        if the answer provided is correct.
                      </div>
                    </v-tooltip>
  
                    <v-tooltip location="top" v-if="i === 4">
                      <template v-slot:activator="{ props }">
                        <v-icon small v-bind="props" style="cursor: pointer; display: flex; align-items: center;">
                          mdi-information-outline
                        </v-icon>
                      ...
                      <div style="text-align: center; max-width: 310px;">
                        When you share a quiz and someone finishes it, 'responses' let you view their answers. 
                        On the business plan, there is no limit to the number of responses you can receive.
                      </div>
                    </v-tooltip>
                  </div>
                </v-list-item-content>
            ...
            <!-- Link to Work Plans for School Tab -->
            <p v-if="selectedTab === 'school'" class="text-h7 mb-1">
              See our 
              <a href="#" @click="selectedTab = 'work'" class="text-info text-decoration-none">
                work plans
              </a> 
