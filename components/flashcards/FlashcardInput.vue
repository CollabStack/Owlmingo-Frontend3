...
<template>
  <v-container>
    <div class="flashcard-section">
      <h1 class="text-h4 font-weight-bold text-primary mb-3 outfit outfit-bold" v-motion
        :initial="{ opacity: 0, y: 20 }" :enter="{ opacity: 1, y: 0, transition: { duration: 600 } }">
        AI Flashcard Generator
      </h1>
      <p class="text-subtitle-1 text-grey-darken-1 mb-6 outfit outfit-regular" v-motion
        :initial="{ opacity: 0, y: 20 }" :enter="{ opacity: 1, y: 0, transition: { duration: 600, delay: 200 } }">
        Upload a document, paste your notes, or select an image to automatically generate flashcards with AI.
      </p>
      ...
      <v-row class="align-center" v-motion :initial="{ opacity: 0, y: 20 }"
        ...
        <v-col cols="9">
          <v-btn-toggle v-model="tab" style="width: 100%; height: 50px;"
      ...
      <v-bottom-sheet v-model="sheet" inset>
        <v-card class="text-center py-5 px-5" rounded="lg">
          <v-card-text class="d-flex justify-space-between align-center">
            <p class="text-h5 text-primary outfit outfit-bold">Flashcard Options</p>
          ...
          <!-- Select Options -->
          <v-container>
            <p class="text-primary text-h6 text-left outfit outfit-medium">Card Style</p>
            <v-select clearable chips label="Classic" :items="['Classic', 'Modern', 'Minimal']"
              class="my-3 animated-input"></v-select>

            <p class="text-primary text-h6 text-left outfit outfit-medium">Max Cards</p>
            <v-select clearable chips label="Auto" :items="['Auto', '10', '20', '30']"
              class="my-3 animated-input"></v-select>
          </v-container>
        </v-card>
      </v-bottom-sheet>
      ...
      <v-window v-model="tab" class="mt-5">
        ...
        <v-window-item value="document">
          <FileUploader icon="mdi-file-upload" iconColor="secondary"
            placeholder="Drag a PDF or DOC file here to create flashcards"
            acceptedFileTypes="application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
            buttonText="Browse Files" buttonIcon="mdi-upload" buttonColor="secondary" :maxFileSize="20"
            ...
            @error="showSnackbar" />
          <p class="mt-2 animated-link outfit outfit-regular">Looking for quizzes instead? Try the <nuxt-link
              to="/quiz" style="text-decoration: none;"> AI Quiz Generator</nuxt-link></p>
        </v-window-item>
        ...
        <v-window-item value="text">
          <div class="ma-4 pa-4" outlined v-motion :initial="{ opacity: 0, y: 20 }"
            ...
            <v-textarea clearable clear-icon="mdi-close-circle" label="Notes"
              placeholder="Paste your study notes here..." hide-details="auto" class="clean-textarea"
              v-model="textContent" rows="8" bg-color="white" color="secondary" counter="4000" variant="plain">
            </v-textarea>
            <div class="button-container">
              <v-btn color="secondary" min-width="92" variant="outlined"
                class="custom-btn text-none animated-btn outfit outfit-medium" @click="processContent"
            ...
            <p class="animated-link outfit outfit-regular mt-4">Looking for quizzes instead? Try the <nuxt-link
                to="/quiz" style="text-decoration: none;"> AI Quiz Generator</nuxt-link></p>
          </div>
        </v-window-item>
        ...
        <v-window-item value="image">
          <FileUploader icon="mdi-image" iconColor="secondary" placeholder="Drag an image containing text here"
            acceptedFileTypes="image/*" buttonText="Browse Images" buttonIcon="mdi-image-search"
            buttonColor="secondary" :maxFileSize="10"
    ...
    <!-- Sign-up Section Below -->
    <!-- <div class="signup-section">
      <v-card class="auth-card pa-6 mt-8">
        <v-card-title class="text-center text-h5 font-weight-bold">
          Sign up for free to continue
        </v-card-title>
        <v-card-subtitle class="text-center">
          Create a free account to continue generating flashcards with AI
        </v-card-subtitle>

        <v-divider class="my-4"></v-divider>

        <v-btn block color="blue darken-1" class="mb-3" @click="signUpWithGoogle">
          <v-icon left>mdi-google</v-icon> Sign Up with Google
        </v-btn>

        <v-btn block color="purple darken-1" class="mb-3" @click="signUpWithEmail">
          <v-icon left>mdi-email</v-icon> Sign Up by Email
        </v-btn>

        <v-card-text class="text-center mt-2">
          Or <nuxt-link to="/login" class="login-link">login</nuxt-link> to an existing account
        </v-card-text>
      </v-card>
    </div> -->
  </v-container>
  ...
  <SubscriptionPlans @subscribe="handleSubscription" @tabChange="handleTabChange" @linkClick="navigateToPlansPage" />
</template>
...
const handleFileSelected = (type, file) => {
  if (type === 'document') {
    ...
    showSnackbar(`Document "${file.name}" uploaded successfully. Ready to process.`);
  } else if (type === 'image') {
    imageFile.value = file;
    showSnackbar(`Image "${file.name}" uploaded successfully. Ready to process.`);
  }
};
...
const processContent = () => {
  if (textContent.value) {
    ...
    setTimeout(() => {
      loading.value = false;
      showSnackbar('Your content has been processed. Ready to generate flashcards.');
    }, 1500);
  }
};
</script>
...
/* Improve focus and hover effects */
...
/* Flashcard and Sign-up section styles */
.flashcard-section {
  position: relative;
  margin-bottom: 2rem;
}
